...
class Categories extends \XoopsObject
{
	...
	public function getFormCategories($action = false)
	{
		...
		// Title
		$title = $this->isNew() ? sprintf(_AM_MYMODULE3_CATEGORY_ADD) : sprintf(_AM_MYMODULE3_CATEGORY_EDIT);
		// Get Theme Form
		xoops_load('XoopsFormLoader');
		$form = new \XoopsThemeForm($title, 'form', $action, 'post', true);
		$form->setExtra('enctype="multipart/form-data"');
		// Form Text catName
		$form->addElement(new \XoopsFormText( _AM_MYMODULE3_CATEGORY_NAME, 'cat_name', 50, 255, $this->getVar('cat_name') ), true);
		// Form Text Date Select catCreated
		$catCreated = $this->isNew() ? 0 : $this->getVar('cat_created');
		$form->addElement(new \XoopsFormTextDateSelect( _AM_MYMODULE3_CATEGORY_CREATED, 'cat_created', '', $catCreated ), true);
		// To Save
