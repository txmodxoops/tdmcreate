...
switch($op) {
	...
	default:
		...
		$templateMain = 'mymodule3_admin_articles.tpl';
		$GLOBALS['xoopsTpl']->assign('navigation', $adminObject->displayNavigation('articles.php'));
		$adminObject->addItemButton(_AM_MYMODULE3_ADD_ARTICLE, 'articles.php?op=new', 'add');
		...
		if ($articlesCount > 0) {
			...
			$GLOBALS['xoopsTpl']->assign('error', _AM_MYMODULE3_THEREARENT_ARTICLES);
		}
	break;
	case 'new':
		$templateMain = 'mymodule3_admin_articles.tpl';
		$GLOBALS['xoopsTpl']->assign('navigation', $adminObject->displayNavigation('articles.php'));
		$adminObject->addItemButton(_AM_MYMODULE3_ARTICLES_LIST, 'articles.php', 'list');
		$GLOBALS['xoopsTpl']->assign('buttons', $adminObject->displayButton('left'));
		// Get Form
	...
	case 'save':
		...
		$uploaderErrors = '';
		$uploader = new \XoopsMediaUploader(MYMODULE3_UPLOAD_IMAGE_PATH . '/articles/', 
		...
		if ($uploader->fetchMedia($_POST['xoops_upload_file'][0])) {
			...
			if (!$uploader->upload()) {
				...
				if ($maxwidth > 0 && $maxheight > 0) {
					...
					$imgHandler->sourceFile    = MYMODULE3_UPLOAD_IMAGE_PATH . '/articles/' . $savedFilename;
					$imgHandler->endFile       = MYMODULE3_UPLOAD_IMAGE_PATH . '/articles/' . $savedFilename;
		...
		$imgNameDef     = Request::getString('art_title');
		$uploader = new \XoopsMediaUploader(MYMODULE3_UPLOAD_FILES_PATH . '/articles/', 
		...
		if ($articlesHandler->insert($articlesObj)) {
			...
			$grouppermHandler->deleteByModule($mid, 'mymodule3_view_articles', $permId);
			if (isset($_POST['groups_view_articles'])) {
				foreach($_POST['groups_view_articles'] as $onegroupId) {
					$grouppermHandler->addRight('mymodule3_view_articles', $permId, $onegroupId, $mid);
				}
			}
			// Permission to submit_articles
			$grouppermHandler->deleteByModule($mid, 'mymodule3_submit_articles', $permId);
			if (isset($_POST['groups_submit_articles'])) {
				foreach($_POST['groups_submit_articles'] as $onegroupId) {
					$grouppermHandler->addRight('mymodule3_submit_articles', $permId, $onegroupId, $mid);
				}
			}
			// Permission to approve_articles
			$grouppermHandler->deleteByModule($mid, 'mymodule3_approve_articles', $permId);
			if (isset($_POST['groups_approve_articles'])) {
				foreach($_POST['groups_approve_articles'] as $onegroupId) {
					$grouppermHandler->addRight('mymodule3_approve_articles', $permId, $onegroupId, $mid);
				}
			}
			if ('' !== $uploaderErrors) {
				redirect_header('articles.php?op=edit&art_id=' . $artId, 5, $uploaderErrors);
			} else {
				redirect_header('articles.php?op=list', 2, _AM_MYMODULE3_FORM_OK);
			}
		}
		// Get Form
	...
	case 'delete':
		...
		if (isset($_REQUEST['ok']) && 1 == $_REQUEST['ok']) {
			...
			if ($articlesHandler->delete($articlesObj)) {
				redirect_header('articles.php', 3, _AM_MYMODULE3_FORM_DELETE_OK);
			} else {
				$GLOBALS['xoopsTpl']->assign('error', $articlesObj->getHtmlErrors());
			}
		} else {
			xoops_confirm(array('ok' => 1, 'art_id' => $artId, 'op' => 'delete'), $_SERVER['REQUEST_URI'], sprintf(_AM_MYMODULE3_FORM_SURE_DELETE, $articlesObj->getVar('art_title')));
		}
	break;
}
